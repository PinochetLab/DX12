...
static VertexPosColor g_Vertices[16] = {
    ...
    { XMFLOAT3( -1.0f + 3.0f, -1.0f, -1.0f ), XMFLOAT3( 0.0f, 0.0f, 0.0f ) },  // 0
    { XMFLOAT3( -1.0f + 3.0f, 1.0f, -1.0f ), XMFLOAT3( 0.0f, 1.0f, 0.0f ) },   // 1
    { XMFLOAT3( 1.0f + 3.0f, 1.0f, -1.0f ), XMFLOAT3( 1.0f, 1.0f, 0.0f ) },    // 2
    { XMFLOAT3( 1.0f + 3.0f, -1.0f, -1.0f ), XMFLOAT3( 1.0f, 0.0f, 0.0f ) },   // 3
    { XMFLOAT3( -1.0f + 3.0f, -1.0f, 1.0f ), XMFLOAT3( 0.0f, 0.0f, 1.0f ) },   // 4
    { XMFLOAT3( -1.0f + 3.0f, 1.0f, 1.0f ), XMFLOAT3( 0.0f, 1.0f, 1.0f ) },    // 5
    { XMFLOAT3( 1.0f + 3.0f, 1.0f, 1.0f ), XMFLOAT3( 1.0f, 1.0f, 1.0f ) },     // 6
    { XMFLOAT3( 1.0f + 3.0f, -1.0f, 1.0f ), XMFLOAT3( 1.0f, 0.0f, 1.0f ) }     // 7
};

static WORD g_Indices[72] = { 0, 1, 2, 0, 2, 3, 4, 6, 5, 4, 7, 6, 4, 5, 1, 4, 1, 0,
                               3, 2, 6, 3, 6, 7, 1, 5, 6, 1, 6, 2, 4, 0, 3, 4, 3, 7,
                                8, 9, 10, 8, 10, 11, 12, 14, 13, 12, 15, 14, 12, 13, 9, 12, 9, 8,
                               11, 10, 14, 11, 14, 15, 9, 13, 14, 9, 14, 10, 12, 8, 11, 12, 11, 15
};
...
void OnUpdate( UpdateEventArgs& e )
{
    ...
    float       yScale           = 1 / tan( Math::Radians(fieldOfView) / 2 );
    float xScale = yScale / aspectRatio;
    projectionMatrix             = XMMatrixSet( 
        xScale, 0, 0, 0, 0, yScale, 0, 0, 0, 0, ( reversed ? n / (f - n) : f / ( f - n ) ),
                                    reversed ? 1 : 1, 0, 0, ( reversed ? n * f / (f-n) : -n * f / ( f - n ) ), 0 
    );
